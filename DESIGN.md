...
- Every state is derived from what the runtime is already doing — processing, an error verdict, a
...
- `prefers-reduced-motion` freezes it on the first frame of the current state, and that frame must
  read on its own.
- One sheet, one image: rows are states, columns are frames, animated with a stepped background
  scroll. No per-frame JavaScript, no sprite runtime library.

If the sprite ever stops answering "what is the system doing right now", it becomes decoration and
must be removed.

## Current MVP Effects Budget

The current MVP uses three noticeable effects:
